...
const FLASH_MATERIAL = preload("uid://b5bgsmx2gvmll")
const FLOATING_TEXT = preload("uid://o3uoptb363sb")
const COINS_SCENE = preload("uid://polxvhd6qmkn")
const ITEM_CARD_SCENE = preload("uid://cc8hgropp6538")
const SELECTION_CARD_SCENE = preload("uid://bpm4rfgqu8pl0")
const SPAWN_ENEMY_SCENE = preload("uid://bd1hkx66ra2sg")

const COMMON_STYLE = preload("uid://bp4ki7xrkgbvr")
const EPIC_STYLE = preload("uid://omcqse0pxm0k")
const LEGENDARY_STYLE = preload("uid://c1n4u34xx2qm6")
const RARE_STYLE = preload("uid://cchvp0wqalg6g")


const UPGRADE_PROPABILITY_CONFIG = {
	"rare": { "start_wave": 2, "base_multi": 0.06 },
	"epic": { "start_wave": 4, "base_multi": 0.02 },
	"legendary": { "start_wave": 7, "base_multi": 0.0023 },
}

const SHOP_PROPABILITY_CONFIG = {
...
const TIER_COLORS: Dictionary[UpgradeTier, Color] = {
	UpgradeTier.RATE: Color(0.0, 0.557, 0.741),
...
var available_players: Dictionary[String, PackedScene] = {
	"Brawler": preload("uid://dp522hjil87en"),
	"Bunny": preload("uid://dw5oeufaq18dm"),
	"Crazy": preload("uid://kov5xvluek83"),
	"Knight": preload("uid://dnmp6w1lf8s3j"),
	"Well Rounded": preload("uid://drjjmx7shlapk"),
}
...
func calculate_tier_propability(current_wave: int, config: Dictionary) -> Array[float]:
	...
	# player luck increases the chance
	...
	print("Wave: %d, Luck: %.1f => Chance: C:%.2f R:%.2f, E:%.2f L:%.2f" % 
...
func select_items_for_offer(item_pool: Array, current_wave: int, config: Dictionary) -> Array:
	...
	var tier_chances := calculate_tier_propability(current_wave, config)
